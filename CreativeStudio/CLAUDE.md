...
| Music in any video (what models really do, beat sync, mix levels) | `MUSIC-WORKFLOW.md` |
| Making a video with Kling (start frame + end frame) | `kling-start-end-frames.md` |
| Using Higgsfield Marketing Studio (modes, avatars, products) | `marketing-studio-modes.md` |
| Writing any client ad (copy, prompt, or shoot) | `advertising-masterclass.md` |
| Writing sales/ad copy (Sabri Suby method) | `COPYWRITING-PLAYBOOK.md` |
| Ad creative for Facebook / Instagram / TikTok | `platform-content-strategy.md` |
...
| The parked auto weekly-content idea (future, not built) | `WEEKLY-CONTENT-FACTORY.md` |
...
- **Every video prompt shown to Cal MUST come with a beat breakdown. HARD RULE.** State how many beats (shots) the clip has and what each beat is, one short line per beat (e.g. "3 beats: 1) waistband snap closeup, 2) scoops bag and heads out the door, 3) wide hallway stride, hands in pockets"). The breakdown is stated SEPARATELY for Cal alongside the char count and scorecard. NEVER write the beat labels or craft notes ("Beat 1", "HELD", "so the snap reads") into the gen prompt itself — the prompt the model receives stays clean cinematic prose; pacing is conveyed in plain description ("fast and snappy, hard cuts only, the snap lingering a moment longer"). Every video prompt, every client. (Locked by Cal, June 13 2026.)
...
- **MUSIC WORKFLOW — LOCKED June 12 2026 (Cal: "don't make that mistake ever again"). HARD RULE.** For ANY clip where motion should ride music: (1) make the track FIRST (Suno, MP3 only — other formats fail silently), (2) upload it as the audio input and NAME it in the prompt — "Use @Audio1 as the rhythmic foundation. Sync cuts and movement changes to its beat positions." — as its own sound block at the END of the prompt, (3) describe NO other music anywhere in the prompt — a music description ("a stadium rock anthem plays") is the instruction that tells Seedance to compose its OWN track and ignore yours (this is what broke the BU03 band clips June 11 2026, envelope correlation ~0), (4) the @Audio1 influence is approximate (~70-80%, mood/tempo) so ALWAYS marry the real track in FFmpeg post and verify by ear. This is Higgsfield's own flagship workflow (their K-Pop case study: music made separately, uploaded before a single frame generated). Full research + sources: `MUSIC-WORKFLOW.md`. NEVER describe music in a video prompt. No exceptions.
...
- **Every action must be GROUNDED in a real place with a real reason. HARD RULE.** Action floating in an empty or abstract space (a kid jumping, running, or crouching against a blank backdrop) reads as weightless, random, and fake AI. Every movement needs a WHERE (a real environment with floor, walls, context, maybe other people in the background) and a WHY (a real situation that motivates it — recess, racing to class, jumping off the last step, playing). Keep the setting bright and uncluttered if the product still needs to read, but never let the action happen in a void. (Locked by Cal, June 13 2026, on the BU03 product video.)
...
- **NEVER stand kids or any AI characters in a row or a line unless Cal explicitly asks for it. HARD RULE.** A line of characters side by side, a lineup at a start line, a tidy row, is the catalog tell — posed, dead, instantly AI. Even "loose" rows read staged. To show a range or multiple characters, scatter them in real activity (a pickup game spread across the floor, kids doing different things at different spots) so the spread reads through motion, never a line. The only exception is when Cal directly asks for a row. (Locked by Cal, June 13 2026, on the BU03 four-color clip.)
