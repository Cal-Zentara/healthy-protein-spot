...
(async () => {
  ...
  if (mode === 'test') {
    const tests = [[0.6,'t-a'],[1.2,'t-b'],[2.0,'t-c'],[3.0,'t-d']];
    for (const [t, name] of tests) {
      await page.evaluate((tt) => window.renderFrame(tt), t);
      await stage.screenshot({ path: path.join(DIR, name + '.png'), omitBackground: true });
    ...
    for (let i = 0; i < total; i++) {
      const t = i / FPS;
      await page.evaluate((tt) => window.renderFrame(tt), t);
      await stage.screenshot({ path: path.join(framesDir, 'f' + String(i).padStart(4,'0') + '.png'), omitBackground: true });
